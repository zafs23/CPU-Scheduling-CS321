...
class CPUScheduling {
	public static void main(String[] args) {
		...
		for (int currentTime = 0; currentTime < simulationTime; currentTime++) {
			...
			if (pGenerator.query()) {
				//maxlevel=priority, maxProcess= processTime, currentTime= arrivaleTime
			...
			if (!pqueue.isEmpty()) {
				...
				//why we are not reseting the timeNotProcessed here, because it doesn't matter if we
				// do it later
				pqueue.update(timeToIncrementLevel, maxLevel);// each iteration has its own timeToIncrementPriority.
				if (arrival)
					System.out.print("\t    ");
				if (next.finish()) {
